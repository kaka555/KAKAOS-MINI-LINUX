...
struct dentry *___add_folder(struct dentry *target_dentry_ptr, const char *folder_name,
                             struct file_operations *file_operations_ptr)
{
                 ...
                 struct file_operations *file_operations_ptr)
{
	...
	return PTR_ERR(___add_folder(target_dentry_ptr, folder_name, file_operations_ptr));
}
...
struct dentry *___add_file(struct dentry *target_dentry_ptr, const char *file_name, struct file_operations *file_operations_ptr)
{
	ASSERT(is_folder(target_dentry_ptr), "make sure is folder\n");
	...
	if (file_operations_ptr)
	{
		inode_ptr = _inode_alloc_and_init(target_dentry_ptr->d_inode->inode_ops,
		                                  file_operations_ptr,
		                                  target_dentry_ptr->d_inode->flag);
	}
	else
	{
		inode_ptr = _inode_alloc_and_init(target_dentry_ptr->d_inode->inode_ops,
		                                  target_dentry_ptr->d_inode->i_f_ops,
		                                  target_dentry_ptr->d_inode->flag);
	}
	if (NULL == inode_ptr)
	{
...
static int __add_file(struct dentry *target_dentry_ptr, const char *file_name, struct file_operations *file_operations_ptr)
{
	...
	return PTR_ERR(___add_file(target_dentry_ptr, file_name, file_operations_ptr));
}
 ...
 */
int remove_all_dentry(struct dentry *root_dentry_ptr)
{
